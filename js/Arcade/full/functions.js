! function($, window, document, _undefined)
{
	...
	XenForo.GameEditor = function($form)
	{
	...
	XenForo.GameEditor.prototype =
	{
		__construct: function($form)
		{
			...
			{
				this.$saveReloadButton.val(this.$saveReloadButton.data('ajaxvalue')).click($.context(this, 'saveAjax'));

				this.$saveExitButton.click($.context(this, 'saveExit'));
		...
		initTitleAndSlug: function()
		{
			...
			/*
			 ...
			 $slug.val($title.val().toLowerCase().replace(/[^a-zA-Z0-9 ]/g,
			 '').trim().replace(/ +/g, '-'));
			 });
			 */
		},
		...
		uploadChange: function(e)
		{
			...
			if ($uploader.val() != '')
			{
				...
				$iframe = $('<iframe src="about:blank" style="display:none; background-color: white" name="AutoGameUploader"></iframe>').insertAfter($uploader).load(function(e)
				{
					var $iframe = $(e.target), ajaxData = $iframe.contents().text(), eComplete = null;
					...
					// removing the iframe after a delay to prevent Firefox' progress indicator
					// staying active
					setTimeout(function()
					{
						$iframe.remove();
				...
				$hiddenInput = $('<span>' + '<input type="hidden" name="_xfNoRedirect" value="1" />' + '<input type="hidden" name="_xfResponseType" value="json-text" />' + '<input type="hidden" name="_xfUploader" value="1" />' + '</span>').appendTo($form);

				$form.attr('target', 'AutoGameUploader').submit().attr('target', '');
		...
		saveAjax: function(e)
		{
			...
			var $files = this.$form.find('input[type=file]')// do this everytime because the
			// form element is changed
			.each(function()
			{
				var v = $(this).val();
				if (v && v.length > 0)
			...
			XenForo.ajax(this.getSaveUrl('json'), postParams, $.context(this, 'ajaxSaveSuccess'));
