				...
				case 'Play':
					...
					if ($activity['params']) {
						if ($activity['params']['id']) {
                                   ...
                                   $game = $gameModel->getGameById($activity['params']['id'], array(
                                        'image' => '0',
                                   ));
...
//			$this->_assertGamePermission('browse');
	...
	public function actionVote() {
		...
		if ($this->_request->isPost() AND ($this->_noRedirect() OR $this->isConfirmedPost())) {
			...
			if ($this->_noRedirect()) {
				$viewParams = array(
					'game' => $this->_getGameModel()->getGameById($game['game_id'], array(
						'voteUserId' => XenForo_Visitor::getUserId()
					)),
				);
				...
				return $this->responseRedirect(
					...
					XenForo_Link::buildPublicLink('arcade/view', $game)
				);
			}
		} else {
			// display the form
			if (!empty($game['voted_date'])) {
				$d = $game['voted_up']?'down':'up'; // revert user old vote
			}
